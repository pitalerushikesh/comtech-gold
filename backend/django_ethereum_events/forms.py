...
class MonitoredEventForm(forms.ModelForm):
    ...
    def clean_contract_abi(self):
        ...
        if not (is_str or is_list):
            raise forms.ValidationError(
                _('Contract abi must be either `str` or `list`'))
    ...
    def clean_event_receiver(self):
        ...
        try:
            ...
            raise forms.ValidationError(_('Cannot import module %(module)s') % {
    ...
    def clean(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                _('Event %(name)s cannot be found in the given contract ABI') % {'name': name})

    def save(self, commit=True):
        event = super(MonitoredEventForm, self).save(
            commit=False)  # event is not ready to be saved

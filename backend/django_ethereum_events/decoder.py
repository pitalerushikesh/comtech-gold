...
class Decoder:
    ...
    def refresh_state(self, block_number):
        ...
        self.topics_by_address.clear()
        self.monitored_events = MonitoredEvent.objects.all()

        for monitored_event in self.monitored_events:
            if monitored_event.contract_address not in self.topics_by_address:
                self.topics_by_address[monitored_event.contract_address] = {}

            self.topics_by_address[monitored_event.contract_address][monitored_event.topic] = monitored_event
    ...
    def decode_log(self, log):
        ...
        event_abi = self.topics_by_address[log.address][log_topic].event_abi_parsed
